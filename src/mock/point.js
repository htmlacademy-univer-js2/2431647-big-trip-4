...
const generateDescription = () => {
  ...
  for (
    let i = 0;
    i < getRandomInteger(ElementsCount.MIN, ElementsCount.MAX);
    i++
  ) {
...
const generateDestination = (id) => ({
  ...
  pictures: Array.from(
    { length: getRandomInteger(ElementsCount.MIN, ElementsCount.MAX) },
    generatePicture
  ),
});
...
const generatePoint = () => {
  ...
  const allOfferIdsByTypePoint = offersByTypePoint.offers.map(
    (offer) => offer.id
  );
  return {
    ...
    offers: Array.from({
      length: getRandomInteger(0, allOfferIdsByTypePoint.length),
    }).map(
      () =>
        allOfferIdsByTypePoint[
          getRandomInteger(0, allOfferIdsByTypePoint.length - 1)
        ]
    ),
